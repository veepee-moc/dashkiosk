...
module.exports = function (app) {
  ...
  app.post('/saved_dashboard', (req, res, next) => {
    // obj : url, description, timeout, delay, availability,
    // watermark, watermarkPosition, groupID, viewport
    if (!req.body && !req.body.url)
      res.status(400).send({ message: 'Bad request' });
    db.SavedDashboards.create(req.body).then(data => res.sendStatus(201))
      .catch(err => next(err));
  });
