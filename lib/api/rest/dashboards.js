  ...
  app.post('/broadcast', (req, res, next) => {
    ...
    groups.forEach(function (groupID, i) {
      models.Group.get(groupID)
        .then(function (group) {
          group.addDashboard(url, params).then(function (dashboard) {
            const currentgroup = dashboard.toJSON().group;
            bus.publish('group.' + currentgroup + '.broadcast', { dashboard: dashboard });
            setTimeout(function () {
              group.getDashboard(dashboard.toJSON().id).then(broadcast => {
                group.deleteDashboard(broadcast);
              });
            }, params.timeout * 1000);
          });
          return (dao);
      ...
      .then(savedDashboard => {
        if (!savedDashboard)
          db.SavedDashboards.create(req.body)
            .then(data => res.status(201).json({ message: 'Dashboard successfully saved.' }))
            .catch(err => next(err));
        else
          res.status(422).json({ message: 'Dashboard already saved.' });
      })
      .catch(err => next(err));
  });
