...
module.exports = function (app) {
  ...
  // TYPES : unassigned
  app.post('/settings/upload/unassigned', image.uploadImageMid('./public/settings/unassigned').single('unassigned'),
  ...
  // Upload brand images
  // TYPES : brand
  app.post('/settings/upload/brand', image.uploadImageMid('./public/settings/brand').single('brand'),
    function (req, res, next) {
      if (!req.file && !req.file.path)
        res.sendStatus(500);
      image.createThumbnail(req.file.path, req.file.filename);
