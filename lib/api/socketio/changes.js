...
function listenChanges(io) {
  ...
  var queue = (function() {
    var p = new BPromise(function(r) { return r(); });         // We don't want group update to be mixed
    ...
    return function(event, group) {
      ...
      // Grab what we need
      p.then(function() {
        return BPromise.all([group.getDashboards(), group.getDisplays()])
  ...
  bus.subscribe('grouptag.*.created', (data) =>
    io.in('changes').emit('grouptag.created', data)
  );
  bus.subscribe('grouptag.*.updated', (data) =>
    io.in('changes').emit('grouptag.updated', data.grouptag.toJSON())
  );
  bus.subscribe('grouptag.*.deleted', (data) =>
    io.in('changes').emit('grouptag.deleted', data.grouptag.toJSON())
  );
}
