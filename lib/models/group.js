...
function Group(name, params) {
  if (name instanceof db.Group.DAO) {
    ...
    this.persistent = db.Group.build({
      name: name,
      rank: params.rank,
      layoutSize: params.layoutSize || 3,
...
Group.prototype.create = function() {
  ...
  function getHighestRank() {
    return db.Group.findAll().then(function (all) {
      var data = _.pluck(all, 'rank');
      var highest = Math.max.apply(Math, data);
      return highest;
    });
  }

  return getHighestRank().then(res => {
    var obj = {
      rank: res + 1,
      name: this.persistent.name,
      description: this.persistent.description
    };
    return db.Group.findOrCreate({ name: this.persistent.name }, obj)
...
Group.prototype.delete = function() {
  ...
  var fixRank = () => {
    models.Group.all().then(groups => {
      var data = _.mapValues(groups, function (gp) { return gp.toJSON(); });
      ...
      for (var i = 0; i < gp.length; i++) {
        if (isAfter)
          mooveRank(gp[i], gp[i].rank - 1);
        if (gp[i].id === self.persistent.id)
          isAfter = true;
      }
    });
  };
...
(function() {
  ...
  Group.prototype.start = function() {
    ...
    function display() {
      ...
      var dispBroadcast = false;
      var dashboardBroadcast;

      if (storage.timer !== null)
        clearTimeout(storage.timer);
      bus.subscribe('group.' + self.persistent.id + '.broadcast', function(data) {
        dispBroadcast = true;
        dashboardBroadcast = data.dashboard;
        next(null, dashboardBroadcast);
      });
      if (!dispBroadcast) {
    ...
    function next(now, dashboardBroadcast) {
        self.getDashboards()
        .then(function(dashboards) {
          if (dashboards.length === 0) {
          ...
          do {
            storage.currentDashboard = dashboardBroadcast ?
            dashboardBroadcast : dashboards[++current] || dashboards[current = 0];
  ...
  return function() {
    if (promise === null) {
      promise = db.Group.findOrCreate({ name: 'Unassigned' },
                                      { rank : 0, description: 'Default group for unassigned displays' })
...
Group.chromecast = (function() {
  ...
  return function() {
    if (promise === null) {
      promise = db.Group.findOrCreate({ rank:1, name: 'Chromecast devices' },
