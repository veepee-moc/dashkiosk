...
Group.all = function() {
  var group_ids = [];
  var groups_list = [];
  return db.Group.findAll({order:[['rank','ASC']]})
    .then(function(groups) {
      _.map(groups, function(g) { group_ids.push(parseInt(g.rank + '' + g.id)); });
      
       //return _.object(_.pluck(groups, 'id'),
       //                _.map(groups, function(g) { return new Group(g); }));
      
       _.map(groups, function(g) { groups_list.push(new Group(g)); });
      
      var res = _.object(group_ids, groups_list);
      return res;
    });
};
