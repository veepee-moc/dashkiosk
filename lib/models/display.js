...
(function() {
  ...
  Display.prototype.disconnect = function() {
    ...
    if (connected[name] <= 0) {
      ...
      bus.publish('display.' + name + '.disconnected',
                  { display: this });
      prometheus.setDisplayStatus(this.persistent, 0);
    }
  };
  ...
  Display.prototype.connect = function() {
    ...
    bus.publish('display.' + name + '.connected',
                { display: this });
    prometheus.setDisplayStatus(this.persistent, 1);
  };
...
function regenMetrics() {
  prometheus.register.resetMetrics();
  Display.all().then(displays => {
    for(const display in displays) {
      if(displays[display].isConnected())
        prometheus.setDisplayStatus(displays[display].toJSON(), 1);
      else
        prometheus.setDisplayStatus(displays[display].toJSON(), 0);
    }
  });
};
